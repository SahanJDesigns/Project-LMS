...
export const GET = async (request: Request) => {
  try {
    ...
    return new NextResponse(JSON.stringify({ lesson }), { status: 200 });
  } catch (error: any) {
    console.error("Error in fetching lesson:", error);
    return new NextResponse(`Error in fetching lesson: ${error.message}`, { status: 500 });
  }
};
